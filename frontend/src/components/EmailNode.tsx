  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2 bg-accent/10 border-b border-border">
        ...
        <span className="text-xs font-semibold text-foreground">Email Node</span>
        {nodeStatus && (
      ...
      <div className="px-3 py-2.5">
        <p className="text-[11px] text-muted-foreground truncate max-w-[200px]">
          to: {data.config?.to || "No recipient set"}
        </p>
        <p className="text-[10px] text-muted-foreground/60 mt-1 truncate max-w-[200px]">
          {data.config?.subject || "Taskpilot notification"}
        </p>
      </div>
      {nodeOutput && (
        <div className="px-3 py-2 border-t border-border bg-secondary/30">
          ...
          <p className="text-[11px] text-foreground/80 break-words">
            {nodeOutput.success ? `Sent to ${nodeOutput.to}` : "Failed"}
          </p>
        </div>
      )}
