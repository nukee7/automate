...
const GithubNode = ({ id, data, selected }: NodeProps) => {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2 bg-accent/10 border-b border-border">
        ...
        <span className="text-xs font-semibold text-foreground">GitHub Trigger</span>
        {nodeStatus && (
      ...
      <div className="px-3 py-2.5">
        <p className="text-[11px] text-muted-foreground truncate max-w-[200px]">
          {nodeOutput
            ? `${nodeOutput.event}: ${nodeOutput.headCommit || nodeOutput.title || nodeOutput.action || ""}`
            : "Waiting for GitHub event"}
        </p>
      </div>
      {nodeOutput && (
        <div className="px-3 py-2 border-t border-border bg-secondary/30">
          <p className="text-[10px] font-medium text-muted-foreground mb-1">
            {nodeOutput.event} {nodeOutput.action ? `(${nodeOutput.action})` : ""}
          </p>
          <p className="text-[11px] text-foreground/80 max-h-[60px] overflow-y-auto break-words whitespace-pre-wrap font-mono">
            {nodeOutput.repository} {nodeOutput.branch ? `→ ${nodeOutput.branch}` : ""}
          </p>
        </div>
      )}
